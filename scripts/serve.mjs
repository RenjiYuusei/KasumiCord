...
export function serve(options) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (pathname?.endsWith(".js")) {
      try {
        ...
        } else if (pathname === "/kasumicord.js") {
          const { config, context, timeTook } = await buildBundle();
  ...
  console.info(chalk.bold.yellowBright("Serving KasumiCord on:"));
  ...
  for (const netinterfaces of Object.values(netInterfaces)) {
    for (const details of netinterfaces || []) {
      ...
      console.info(`  http://${details.address}:${port}/kasumicord.js`);
      console.info(`  http://${details.address}:${port}/kasumicord.min.js`);
    }
  }
